 ...
 * de la @ tenga al menos un punto 
 * Que entre la @ y el punto a su derecha haya
 * una separación de dos caracateres 
 * Que despues del último punto haya entre 2 y
 * 6 caracteres
 */
public class ValidarMail2 {
	...
	public static void main(String[] args) {
		...
		} else {System.out.println("no tiene espacios");}
		//Que despues del último punto haya entre 2 y 6 caracteres
		int posicionPunto = email.lastIndexOf(".");
		String subEmail = email.substring(posicionPunto);
		int longitudSubEmail = subEmail.length();
		...
		else {System.out.println("esta entre los valores 2 y 6");}
		// que el resultado de la busqueda de la posición  de @ tanto por el principio y por el final sea el mismo y que la posición de la @ sea menor que la del punto.
		
		if ( email.indexOf("@")!=email.lastIndexOf("@") ) {
			mensajeDeError += "- solo puede haber una @ .\n";
			System.out.println(mensajeDeError);

			} else {System.out.println("solo tiene una @");}
		if (!email.contains("@")) {
